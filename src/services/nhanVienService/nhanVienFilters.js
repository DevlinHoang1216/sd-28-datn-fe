...
export const buildAPIParams = (filters, activeTab, tabs, currentPage, pageSize) => {
  ...
  // Add gender filter
  if (filters.gioiTinh !== '') {
    params.gioiTinh = filters.gioiTinh === 'true'
  }
...
export const filterEmployees = (allEmployees, filters, activeTab, tabs) => {
  // This function is deprecated - all filtering is now done by the API
  return []
}

export const paginateEmployees = (filteredEmployees, currentPage, pageSize) => {
  const start = currentPage * pageSize
  const end = start + pageSize
  return filteredEmployees.slice(start, end)
}

export const calculateTotalPages = (filteredEmployees, pageSize) => {
  return Math.ceil(filteredEmployees.length / pageSize) || 1
}

export const getTabCount = (allEmployees, tabId, tabs) => {
  if (!allEmployees.length) return 0
  if (tabId === 'all') return allEmployees.length
  const tab = tabs.find(t => t.id === tabId)
  if (tab?.status === undefined) return 0
  return allEmployees.filter(emp => emp.trangThai === tab.status).length
}
...
export const resetFilters = (filters, tempFilters, toast, refreshCallback) => {
  const defaultFilters = nhanVienConfig.defaultFilters
