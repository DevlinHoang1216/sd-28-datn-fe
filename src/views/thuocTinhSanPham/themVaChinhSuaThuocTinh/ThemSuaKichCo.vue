<template>
  <div class="them-kich-co-container">
    <!-- Breadcrumb -->
    <Breadcrumb 
      :items="breadcrumbItems"
      :show-page-info="true"
      page-title="Thêm Kích Cỡ Mới"
      page-description="Thêm kích cỡ mới cho sản phẩm"
...
export default {
  ...
  setup() {
    ...
    const breadcrumbItems = ref([
      ...
      { label: 'Thêm mới', path: '/thuoc-tinh/kich-co/them' }
    ]);
    ...
    // Methods
    const loadSize = async (id) => {
      try {
        loading.value = true;
        const response = await productService.getSizeById(id);
        ...
        breadcrumbItems.value[3] = { label: 'Chỉnh sửa', path: `/thuoc-tinh/kich-co/sua/${id}` };
      } catch (error) {
        toast.error('Lỗi khi tải thông tin kích cỡ');
    ...
    const saveSize = async () => {
      try {
        ...
        try {
          const checkResponse = await productService.checkSizeNameExists(
            sizeForm.value.name.trim(),
            isEdit.value ? sizeId.value : null
          );
        ...
        const sizeData = {
          tenKichCo: sizeForm.value.name.trim()
        };

        if (isEdit.value) {
          await productService.updateSize(sizeId.value, sizeData);
    ...
    const goBack = () => {
      router.push('/thuoc-tinh/kich-co');
    };
    ...
    onMounted(() => {
      if (route.params.id) {
        isEdit.value = true;
        sizeId.value = route.params.id;
        loadSize(route.params.id);
      }
    });
