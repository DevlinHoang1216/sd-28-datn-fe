<template>
  <div class="them-thuong-hieu-container">
    <!-- Breadcrumb -->
    <Breadcrumb 
      :items="breadcrumbItems"
      :show-page-info="true"
      page-title="Thêm Thương Hiệu Mới"
      page-description="Thêm thương hiệu mới cho sản phẩm"
...
export default {
  ...
  setup() {
    ...
    const breadcrumbItems = ref([
      ...
      { label: 'Thêm mới', path: '/thuoc-tinh/thuong-hieu/them' }
    ]);
    ...
    const loadBrand = async (id) => {
      try {
        loading.value = true;
        const response = await productService.getBrandById(id);
        ...
        breadcrumbItems.value[3] = { label: 'Chỉnh sửa', path: `/thuoc-tinh/thuong-hieu/sua/${id}` };
      } catch (error) {
    ...
    const saveBrand = async () => {
      try {
        ...
        try {
          const checkResponse = await productService.checkBrandNameExists(
            brandForm.value.name.trim(),
            isEdit.value ? brandId.value : null
          );
        ...
        const brandData = {
          tenThuongHieu: brandForm.value.name.trim()
        };

        if (isEdit.value) {
          await productService.updateBrand(brandId.value, brandData);
    ...
    const goBack = () => {
      router.push('/thuoc-tinh/thuong-hieu');
    };
